              ...
              <div class="flex justify-between items-center mb-1">
                <h4 class="font-semibold text-primary">
                  ...
                  <span class="text-sm text-gray-500 ml-2">
                    {{ formatDate(commentaire.date) }}</span
                  >
                </h4>
      ...
      <div>
        ...
        <form @submit.prevent="soumettreAvis" class="space-y-4">
          <div class="flex items-center gap-2">
            ...
            <div class="flex items-center gap-1">
              <Icon
                v-for="i in 5"
                :key="i"
                :icon="i <= nouvelleNote ? 'mdi:star' : 'mdi:star-outline'"
                class="text-yellow-400 text-2xl cursor-pointer"
                @click="nouvelleNote = i"
              />
            </div>
          </div>
          ...
          <textarea
            ...
            rows="5"
            class="bg-gray-50 w-full border border-gray-300 rounded-xl px-4 py-2 focus:outline-none focus:ring-2 focus:ring-primary"
          ></textarea>
...
const soumettreAvis = async () => {
  ...
  try {
    ...
    const token = getToken() // Utiliser la fonction getToken()
